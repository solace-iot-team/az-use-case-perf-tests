...
echo " >>> Starting Load ..."
  $scriptDir/start.aws.load.sh > $scriptDir/start.aws.load.log
  if [[ $? != 0 ]]; then echo ">>> ERROR starting load"; echo; exit 1; fi

echo " >>> Running Monitors ..."
  $scriptDir/monitor/run.aws.monitor.sh auto
  if [[ $? != 0 ]]; then echo ">>> ERROR running monitors"; echo; exit 1; fi

echo " >>> Stop Load ..."
  $scriptDir/stop.aws.load.sh > $scriptDir/stop.aws.load.log
