...
resultDirLatest="$resultDirBase/aws/run.latest"
...
echo ">>> Start VPN Stats Monitor ..."
  $scriptDir/run.aws.monitor.vpn-stats.sh 2>&1 > $scriptDir/run.aws.monitor.vpn-stats.log &
  aws.vpn_stats_pid=" $!"
  pids+=" $!"

# echo "##############################################################################################################"
echo ">>> Start Latency Stats Monitor ..."
  $scriptDir/run.aws.monitor.latency.sh 2>&1 > $scriptDir/run.aws.monitor.latency.log &
  aws.latency_pid=" $!"
  pids+=" $!"

# echo "##############################################################################################################"
echo ">>> Start Latency Broker Node Stats Monitor ..."
  $scriptDir/run.aws.monitor.brokernode.latency.sh 2>&1 > $scriptDir/run.aws.monitor.brokernode.latency.log &
  aws.brokernode_latency_pid=" $!"
  pids+=" $!"

# echo "##############################################################################################################"
echo ">>> Start Ping Latency Stats Monitor ..."
  $scriptDir/run.aws.monitor.ping.sh 2>&1 > $scriptDir/run.aws.monitor.ping.log &
  aws.ping_pid=" $!"
  pids+=" $!"
