...
echo ">>> Start VPN Stats Monitor ..."
  $scriptDir/run.az.monitor.vpn-stats.sh 2>&1 > $scriptDir/run.azs.monitor.vpn-stats.log &
  az.vpn_stats_pid=" $!"
  pids+=" $!"

# echo "##############################################################################################################"
echo ">>> Start Latency Stats Monitor ..."
  $scriptDir/run.az.monitor.latency.sh 2>&1 > $scriptDir/run.az.monitor.latency.log &
  az.latency_pid=" $!"
  pids+=" $!"

# echo "##############################################################################################################"
echo ">>> Start Latency Broker Node Stats Monitor ..."
  $scriptDir/run.az.monitor.brokernode.latency.sh 2>&1 > $scriptDir/run.az.monitor.brokernode.latency.log &
  az.brokernode_latency_pid=" $!"
  pids+=" $!"

# echo "##############################################################################################################"
echo ">>> Start Ping Latency Stats Monitor ..."
  $scriptDir/run.az.monitor.ping.sh 2>&1 > $scriptDir/run.az.monitor.ping.log &
  az.ping_pid=" $!"
  pids+=" $!"
